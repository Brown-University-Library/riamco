class Collections
    def self.all
        Rails.cache.fetch("collections_all", expires_in: 2.minute) do
            ...
            params.q = "inventory_level_s:Collection"
            params.page_size = 10000 ## TODO: make this value configurable
            results = solr.search(params)
            docs = {}
            results.solr_docs.each do |doc|
                key = doc["id"]
                # TODO: convert these docs to Collection objects.
                docs[key] = doc 
            end
            docs 
        end
    rescue Exception => e
        Rails.logger.error "Collections could not be fetch: #{e.message}"
        {}
    end
